...
public class AdminLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (email != null && password != null) {
			rs = AdminDao.validate(email, password);
			try {
				if (rs.next()) {
					...
					out.println("<h1>Welcomes To Admin Pannel</h1>");
					...
					/*
					 * request.getRequestDispatcher("adminhome.html").include(request,
					 * response);
					 */
				} else {
					request.getRequestDispatcher("navhome.jsp").include(request, response);
					out.println("<div class='main'>");
					out.println("<section class='sign-up'>");
					out.println("<div class='container'>");
